...
if __name__ == "__main__":
    ...
    if not 0 <= len(sys.argv) <= 52:
        ...
        sys.stderr.write("Usage: %s <BBC level dir> <[scale]\n" % sys.argv[0])
        sys.exit(1)
    ...
    for level_number in range(1, 25):
        ...
        level_image.putpalette((0,0,0, 255,0,0, 0,255,0, 0,0,255))
        if level_image.mode != 'RGB':
            print("Converting level image to RGB mode for scaling.")
        ...
        for row in range(48):
            for column in range(32):
                ...
                if image_name == level_number:
                    im = Image.open(os.path.join("tmp", str(level_number).zfill(2)+ ".bmp"))
                    im = hqx.hq2x(im)
